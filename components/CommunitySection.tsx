...
const BENEFITS = [
  { icon: Users, title: "Network", desc: "Connect with 1,000+ BAs worldwide" },
  { icon: MessageCircle, title: "Support", desc: "Get answers from experienced analysts" },
  { icon: TrendingUp, title: "Growth", desc: "Career resources and mentorship" },
  { icon: BookOpen, title: "Resources", desc: "Exclusive guides and templates" },
  { icon: Heart, title: "Belonging", desc: "A community that actually cares" },
  { icon: Zap, title: "Opportunities", desc: "Jobs, gigs, and collaborations" },
];

const ROLES = [
  "Student", "Junior BA", "Business Analyst", "Senior BA",
  "Product Owner", "Project Manager", "Other"
];

export function CommunitySection() {
  const [success, setSuccess] = useState(false);

  const { register, handleSubmit, reset, setValue, formState: { errors } } = useForm<JoinCommunityData>({
  ...
  const onSubmit = async (data: JoinCommunityData) => {
    try {
      ...
      if (typeof window !== "undefined") {
        ...
        confetti({ particleCount: 150, spread: 80, origin: { y: 0.6 }, colors: ["#F59E0B", "#2563EB", "#0F1F3D"] });
      }
  ...
  return (
    <section id="community" className="py-28 bg-soft-white relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <span className="text-electric text-sm font-semibold tracking-[0.2em] uppercase">Join Us</span>
          <h2 className="font-cormorant text-5xl sm:text-6xl font-bold text-navy mt-3 mb-5">
            The Practical BA Community
          </h2>
          <p className="text-charcoal/50 max-w-2xl mx-auto">
            A space to learn, grow, and connect with fellow BAs on the same journey. Thousands strong and growing.
          </p>
        </motion.div>
              ...
              <>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
                  <div className="grid sm:grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="text-xs font-semibold text-charcoal/60 uppercase tracking-wide mb-1.5 block">Last Name</label>
                      <input
                        {...register("lastName")}
                        className="w-full px-4 py-3 rounded-xl border border-charcoal/10 focus:border-electric focus:ring-2 focus:ring-electric/10 outline-none transition-all"
                      />
                  ...
                  <div>
                    <label className="text-xs font-semibold text-charcoal/60 uppercase tracking-wide mb-1.5 block">Current Role</label>
                    <select
                    ...
                    >
                      ...
                      {ROLES.map((r) => <option key={r} value={r.toLowerCase().replace(" ", "-")}>{r}</option>)}
                    </select>
                    {errors.currentRole && <p className="text-red-500 text-xs mt-1">{errors.currentRole.message}</p>}
                  </div>
                  <div>
                    <label className="text-xs font-semibold text-charcoal/60 uppercase tracking-wide mb-1.5 block">Biggest BA Challenge</label>
                    <textarea
                      {...register("challenge")}
                      rows={4}
